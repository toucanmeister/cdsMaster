...
#pragma omp parallel for collapse(2) firstprivate(l_param)
  for( int64_t l_kb = 0; l_kb < l_sizes.kb; l_kb++ ) {
    for( int64_t l_nb = 0; l_nb < l_sizes.nb; l_nb++ ) {
      size_t l_c_block_size = l_sizes.bn*l_sizes.bk;
      size_t l_offset_c = l_kb*l_sizes.nb*l_c_block_size + l_nb*l_c_block_size;

      for( int64_t l_cb = 0; l_cb < l_sizes.cb; l_cb++ ) {
        size_t l_a_block_size = l_sizes.bn*l_sizes.bc;
        size_t l_offset_a = l_nb*l_sizes.cb*l_a_block_size + l_cb*l_a_block_size;

        size_t l_b_block_size = l_sizes.bk*l_sizes.bc;
        size_t l_offset_b = l_kb*l_sizes.cb*l_b_block_size + l_cb*l_b_block_size;
