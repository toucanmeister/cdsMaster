...
A distribution, a opposed to a point estimate, provides extra information about how sure we can be of the results of our inference. 
...
def spike_likelihood(x, dim):
    slab = 50 * multivariate_normal.pdf(x, mean=np.zeros(dim), cov=np.eye(dim)*2)
    spike = multivariate_normal.pdf(x, mean=np.zeros(dim), cov=np.eye(dim)*0.01)
    return slab + spike
...
\end{lstlisting}
\hrule
%TODO: Add images of 2D example problems

Each example uses a uniform prior on $[-5, 5]$ over all dimensions.
...
\begin{itemize}
    \item Discretized LRPS (only for 2D problems, with 6000 points per dimension)
    \item Metropolis Algorithm (with simple adaptive step size)
    \item HMC with reflection (only for standard Nested Sampling, with static HMC path length of 3 and step size of 0.5)
...
\end{itemize}
Each configuration of example problem, algorithm and LRPS method was run 150 times, with each run using 10000 live points and a stopping criterion of $\epsilon = 10^{-6}$.
An exception to this are the configurations using discretized LRPS, which could only be done with 1000 live points and $\epsilon = 10^{-4}$, because the number of discrete points required for the original configuration exceeds my memory limits.
