...
%TODO: Deutsche Zusammenfassung schreiben
...
Bayesian statistical models are widely used in traditional small data settings.
They provide a way to extract information from data that leads to intuitively interpretable results.
Because the outcome of a Bayesian model fitting comprises not just the most likely parameter values (as it would in a maximum likelihood model), but a whole probability distribution over all possible parameter values, it is also easy to compute error bounds.
...
Many techniques have been developed for this purpose, with all state-of-the-art approaches being Monte Carlo methods.
The most prevalent ones are Gibbs sampling \cite{gibbs}, Metropolis Hastings \cite{metropolishastings} and Hamiltonian Monte Carlo (HMC) \cite{hmc}.
All of these come with some downsides, however.

A downside that all of these methods share is that they focus purely on the posterior distribution of a model fit and do not compute the evidence term.
The evidence is a useful tool that can be used to compare whole classes of models with respect to how well they fit some data.
Nested sampling \cite{skilling} is another Monte Carlo approach whose main objective is computing the evidence, although it also generates samples from the posterior.
This makes it unique among methods for Bayesian model fitting.
Nested sampling does however have the problem of not being able to use the gradient information of the likelihood in its sampling, which other successful methods like HMC rely heavily upon.
This makes it difficult to explore the posterior efficiently.

In this thesis, I propose a modified version of Nested Sampling which uses ideas from convex optimization to achieve higher acceptance rates during the crucial likelihood-restricted prior sampling step of Nested Sampling.
I begin by recapping important background information on the methods and ideas this thesis is based on, including Bayesian statistics, the Nested Sampling algorithm and the barrier method for solving convex optimization problems.
...
In classical formal logic, we would say ``$\theta = 0.5$'' is true.

But since even the most accurately manufactured coin will have some asymmetries, we can never be sure of the coin's fairness.
...
In doing this, we create a probability distribution over $\theta$.
...
But to make the example a bit more interesting, let us say that by the shape of the coin, we would assume that it would land tails more often than heads.
...
It is called a prior since it encapsulates our beliefs about the model before we have seen any data.
The name giving process of Bayesian statistics is updating our beliefs about the model's parameter values after seeing new data using Bayes' rule (Equation \ref{eq:bayes}).
...
Bayes' rule is made up of four parts.
...
I abbreviate the natural logarithm of the likelihood as $l(\theta)$.

\textbf{Prior}: Bayes' rule requires a prior distribution over the model parameters, $P(\theta)$, often written $\pi(\theta)$.
This represents beliefs we have about the parameter values before taking the data into account, which could be results from previous experiments or qualitative knowledge about the modeled process (e.g. "We know this relationship is positive. Therefore, this slope parameter's prior is zero for negative values.") \cite[34 f.]{mcelreath}.
...
For our simple, one-dimensional example we approximate the posterior numerically by discretizing the $\theta$-space and evaluating Bayes' rule for each discrete point.
    ...
    \begin{minipage}[c]{0.45\textwidth}
        \begin{tikzpicture}
            \begin{axis} [
                ...
                xlabel=$\theta$,
                ylabel=$\pi(\theta)$,
                y label style={at={(axis description cs:-0.1, 0.5)},anchor=east},
    ...
    \begin{minipage}[c]{0.45\textwidth}
        \begin{tikzpicture}
            \begin{axis} [
                ...
                xlabel=$\theta$,
                ylabel=$L(\theta)$,
                y label style={at={(axis description cs:-0.22, 0.5)},anchor=east},
                x label style={at={(axis description cs:0.5,-0.1)},anchor=north},
                scale=0.7,
                yticklabel style={
...
Having introduced Bayesian statistics and described how Bayesian model fitting works conceptually, let us now turn our attention to how to do it in practice.

In some cases, the posterior can be computed analytically, yielding a parameterized probability distribution.
However, this only works for very simple models and limits the types of prior one can use (the prior has to be ``conjugate'' to the likelihood) \cite[39]{mcelreath}.
...
This approach has the drawback of its computational requirements scaling exponentially with the number of parameters.
...
Generating samples from a general distribution is still a difficult task.
...
Another fact to consider is that we do not actually the probability density function (PDF) of the distribution we want to sample from.
...
A relatively simple way to sample from a distribution when we can evaluate an unnormalized version of its density function is rejection sampling.
I will present a quick overview of it to give an example of a Monte Carlo method.

Like most other Monte Carlo methods, rejection sampling makes use of a proposal distribution.
Given an unnormalized density function $\mathbf{P}(\theta)$ of the target distribution we want to sample from, rejection sampling requires a distribution we can sample from and whose density $\mathbf{Q}(\theta)$ we can scale up using a constant $c$ to fully envelop the target density:
\begin{equation}
...
Rejection sampling fails in practice because
\begin{enumerate}
    \item we normally do not know the modes of our target distribution and thus do not know how to choose $c$, and
    \item even if we did know how to choose $c$, the fact that in high dimensional problems only a small fraction of the parameter space carries significant posterior mass forces us to scale $\mathbf{Q}$ so much everywhere else that acceptance rates plummet.
\end{enumerate}
 
The most used algorithms in Bayesian statistics try overcome these problems by choosing a proposal distribution that depends on the previous sample.
They are a subcategory of Monte Carlo methods called Markov chain Monte Carlo (MCMC).
Each MCMC sample is dependent on its predecessor and is often close to it.
This allows more efficient exploration of posterior mass, because once a region of high posterior density is found, its local surroundings often also carry high posterior density. 

Again, let us take a look at one relatively simple example algorithm, the Metropolis algorithm.
The Metropolis algorithm uses a symmetrical proposal distribution $\mathbf{Q}(\theta^{\textrm{new}} \,|\, \theta^{\textrm{old}})$.
Often, a simple normal distribution around the last sample is used.
The proposal is then accepted with probability $\frac{\mathbf{P}(\theta^{\textrm{new}})}{\mathbf{P}(\theta^{\textrm{old}})}$ \cite[365f.]{mckay}.
...
Markov chains are defined as a set of states $\Theta$ (in our case these are all possible parameter values) with an initial probability distribution $P^0(\theta)$ over these states, along with a transition probability function $T: \Theta \times \Theta \rightarrow [0,1]$ assigning a probability to every transition from one state to another.
We can simulate a Markov chain by sampling an initial state $\theta_0 \sim P^0(\theta)$, then sampling the next state from the transition probabtility distribution $\theta_k \sim T(\theta_k \,|\, \theta_{k-1})$.
We can also analyse the probability of being in a certain state after $k$ steps \cite[372]{mckay}: 
$$
    P^k(\theta^*) = \int_{\Theta} P^{k-1}(\theta) \, T(\theta^* \,|\, \theta) \, d\theta 
$$
...
$$
    \mathcal{P}(\theta^*) = \int_{\Theta} \mathcal{P}(\theta) \, T(\theta^* \,|\, \theta) \, d\theta
$$
All MCMC methods revolve around cleverly defining their transition probabilities in such a way that the target distribution is the only stationary distribution of the Markov chain.
...
Now that I've touched on how Bayesian models are normally fit, I want to explain why there is room for improvement.
...
MCMC methods only generate samples from the posterior, they do not compute the evidence.
Computing the evidence integral is generally a difficult task.
Since the parameter space for real-world models is very high-dimensional, with only a small fraction of this space containing meaningful posterior probability mass, numerical quadrature methods fail.
...
There are methods that estimate the evidence from given MCMC samples \cite{bayesian_cubature}, which by definition concentrate where there is high posterior mass.
However, this thesis focuses on a method whose main objective is computing the evidence.
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Visualization of the relationship between a likelihood threshold $\lambda$ and the corresponding prior mass $\mu(\lambda)$ for a uniform prior (purple) and a likelihood (blue) in two dimensions.}
    \label{fig:xlambda}
\end{figure}
...
Then we take the one with the lowest likelihood, remove it from the live points and use it along with an estimate of its prior mass as the first contribution to the evidence.
...
To be sure of its correctness, we have to show that the prior masses of these samples from the likelihood-restricted prior are still uniformly distributed; a proof of this can be found in the Appendix.
...
The number of iterations required to fulfill the same stopping threshold scales linearly with the number of live points. %TODO: MORE DETAILS ON CONVERGENCE RATE WITH SOURCE
See Algorithm \ref{alg:nested_sampling} for an overview of the Nested Sampling procedure.
...
It is also where the original paper by Skilling provides little guidance, only that "such points will usually be found by some MCMC approximation, ..." \cite[6]{skilling}
...
The standard Metropolis algorithm provides no way to adapt the step size during sampling, although more advanced methods have been developed to achieve this \cite{automala}.
...
\begin{figure}
    \centering
    \includegraphics[scale=0.8]{hmc_reflection_example.png}
    \caption{A run of HMC for LRPS with reflection and a chain of five samples on a standard-normal-likelihood-restricted uniform prior space.
...
Because the likelihood-constraint grows tighter, finding samples in the valid area becomes more difficult with every Nested Sampling iteration.
For methods like rejection sampling and the Metropolis algorithm, this manifests in low acceptance rates of proposals, which leads to wasted computational work.
...
We would like an approach where the sampler can ``feel'' the likelihood-constraint before it crosses it.
Thinking back to the barrier method, we can make the following observation.
By approximating the hard constraint function, the log barrier also lets the optimizer ``feel'' the constraint before crossing it.
...
With that, we have succesfully defined a version of Nested Sampling that incorporates the log barrier term from \ref{eq:log_barrier_term}.
...
We will discuss how $t$ and $q_{\textrm{max}}$ practically influence the algorithm's performance in Section %TODO: SECTION HERE.
