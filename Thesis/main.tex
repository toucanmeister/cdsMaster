...
We do the same, taking the logarithm of both the objective function and both sides of the likelihood constraint.    %TODO: Figure out whether to end on a colon or a full stop before an equation
...
The two examples are taken from Skilling's introductory paper on Nested Sampling, where the $L_1$-example is presented as being a type of problem Nested Sampling is well-suited for, while the $L_2$-example is said to be difficult even for Nested Sampling \cite[21]{skilling}.
...
\begin{itemize}
    ...
    \item an optimal method for sampling inside an n-sphere (only for the $L_1$-problem)
\end{itemize}
