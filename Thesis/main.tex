...
As covered in Section 1.1, the evidence is an immensely useful part of Bayes' formula (see (\ref{eq:bayes})) because it allows model comparison that accounts for overfitting.
...
\begin{figure}
    ...
    \caption{Visualization of the Metropolis algorithm in a two-dimensional sample space. The dashed line indicates the likelihood constraint boundary and each cross is a sampled point.}
...
The worse performance of all methods can be explained by the next criteria, concerning how well the posterior distribution was explored.
...
\begin{figure}
    ...
    \caption{Box plot of the deviation of each method's evidence estimate's from the true value. Orange lines are the median values, gray crosses are the mean values.}
...
HMC with reflection sometimes not even finding the flat mode shows that the method fails critically.
\begin{figure}
    \centering
    ...
    \caption{Bar charts of how often each number of modes was found by each configuration.}
...
\begin{figure}
    ...
    \caption{Box plots of how many iterations each method took to reach the stopping point. Orange lines are the median values, gray crosses are the mean values.}
...
\begin{figure}
    ...
    \caption{Box plots of the ratio of accepted proposals to total proposals made during LRPS for each method. Orange lines are the median values, gray crosses are the mean values.}
...
For the HMC methods, the adaptive step size keeps the acceptance rate steadily at $0.5$, so we have to look at how much the length of the proposal Markov chain has to be reduced to achieve this acceptance rate.
We measure this using the jump distance \cite{rjd}, the distance from the start to the end of the proposal Markov chain of each accepted sample.
...
\begin{figure}
    ...
    \includegraphics[scale=0.8]{figs/results/mean_jump_distances_spike_20d.png}
    \includegraphics[scale=0.8]{figs/results/mean_jump_distances_spike_offcenter_20d.png}
    \caption{Mean jump distance of each configuration per iteration. Each data point is the mean over the jump distances of 200 configurations at that iteration. A uniformly weighted rolling average with a window of 50 was applied. Curves are only shown as far as the shortest run went. Note the logarithmic y-axis.}
...
\begin{figure}
    \centering
    {\small \textbf{Evidence Estimate} \par}
    \includegraphics[height=0.95\textwidth]{figs/results/params/logZ_diffs_metropolis.png}
    \includegraphics[height=0.95\textwidth]{figs/results/params/logZ_diffs_hmc.png}
...
\begin{figure}
    \centering
    {\small \textbf{Posterior Modes Found} \par}
    \includegraphics[height=0.95\textwidth]{figs/results/params/modes_found_metropolis.png}
    \includegraphics[height=0.95\textwidth]{figs/results/params/modes_found_hmc.png}
    \caption{Bar charts of how often each number of modes was found by each configuration. Left: Using Metropolis for LPRS, Right: Using HMC for LRPS.}
...
\begin{figure}
    \centering
    {\small \textbf{Number of Iterations} \par}
    \includegraphics[height=0.95\textwidth]{figs/results/params/iterations_metropolis.png}
    \includegraphics[height=0.95\textwidth]{figs/results/params/iterations_hmc.png}
...
In the HMC version, all configurations maintain an acceptance rate close to $0.5$.
\begin{figure}
    \centering
    {\small \textbf{Acceptance Rate} \par}
    \includegraphics[height=0.95\textwidth]{figs/results/params/acceptance_rates_metropolis.png}
    \includegraphics[height=0.95\textwidth]{figs/results/params/acceptance_rates_hmc.png}
...
\subsection{Jump Distance}
For the Metropolis version in Figure \ref{fig:results_params_jump_distances_metropolis}, the value of $t$ has no discernible impact on the jump distances.
...
\begin{figure}
    ...
    \caption{Mean jump distance of each Metropolis configuration per iteration. Each data point is the mean over the jump distances of 200 configurations at that iteration. A uniformly weighted rolling average with a window of 50 was applied. Curves are only shown as far as the shortest run went.}
...
\begin{figure}
    ...
    \caption{Mean jump distance of each HMC configuration per iteration. Each data point is the mean over the jump distances of 200 configurations at that iteration. A uniformly weighted rolling average with a window of 50 was applied. Curves are only shown as far as the shortest run went.}
