...
Commonly used methods (Gibbs sampling \cite{gibbs}, Metropolis Hastings \cite{metropolishastings}, Hamiltonian Monte Carlo (HMC) \cite{hmc}, Slice Sampling \cite{slice_sampling}, etc.) all come with one downside:
...
For a given unnormalized density function $\mathbf{P}(\theta)$ of the target distribution, the proposal distribution's density $\mathbf{Q}(\theta)$ can be scaled up using a constant $c$ to fully envelop the target density:
\begin{equation}
...
The proposal is then accepted with probability $\textrm{max}\left(\frac{\mathbf{P}(\theta_{\textrm{new}})}{\mathbf{P}(\theta_{\textrm{old}})}, 1\right)$ \cite[p. 365f.]{mckay}.
...
We could use $\mathcal{L}$ to compute their likelihood thresholds and sort them from lowest to highest.
...
\begin{itemize}
    ...
    \item and $\mathcal{D}_{\textrm{KL}}$ is the Kullback-Leibler Divergence \cite[6]{kld} between prior and posterior \cite[1]{nested_sampling_talk}.
\end{itemize}
...
\begin{figure}
    ...
    \caption{Visualization of the Metropolis algorithm in a two-dimensional sample space. The dotted line indicates the likelihood constraint boundary, the dot is the starting point and each cross is a sampled point.}
...
Since $q$ is sampled from $\pi_q(q) \, \mathbbm{1}_{\{q < L(\theta) / L^*\}}$, the $\theta$ that are further away from the likelihood constraint are expected to have higher $q$ values, reducing their likelihood more.
...
\end{itemize}
We can use the optimal $d$-ball sampler because the valid region in the $L_1$-problem is always a ball.
