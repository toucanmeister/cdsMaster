...
Dieser Ansatz erreicht in durchgeführten Tests im likelihoodbeschränkten Sampling des Priors Akzeptanzraten die bis zu einem Faktor von 1,23 über der von Standard-Nested-Samping liegen.
...
We also multiply the objective function by $-1$ so we can minimize instead of maximizing, to match the optimization problems from Chapter 1.5: %TODO: Figure out when to say Chapter and when to say Section
\begin{align}
    &\textrm{maximize} \quad \pi(\theta) &&\textrm{subject to} \quad L(\theta) > L^* \nonumber \nonumber \\
...
\end{align*}
With $\mathcal{N}_1$ and $\mathcal{N}_2$ being the weighted mode PDFs, $\theta_1, ..., \theta_n$ being the dead points, $w_1, ..., w_n$ being the dead points' estimated prior mass shells, m being the vector of true mode weights, $(100, 1)$ and $||\cdot||_1$ being the $L^1$-Norm: $||x||_1 = \sum_i |x_i|$.
...
\subsubsection{Number of Iterations} %TODO: REPLACE THESE WITH THE NEWER VERSIONS AFTER PULLING THE NEW t5q11 DATA FROM ARA
...
Using HMC for LRPS, it %TODO: step size results
